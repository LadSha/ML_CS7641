...
def XGB_experiement():
    modelDT = XGBClassifier(random_state=0)
    # find_alph( modelDT)
    # GrSearch()
    classfier_name="XGBlassifier"
    prepare_val_curve(modelDT, 'max_depth', np.arange(1, 40, 1), metric, classfier_name, x_train, y_train)
    prepare_val_curve(modelDT, 'max_leaf_nodes', np.arange(1, 40, 1), metric, classfier_name, x_train, y_train)
    create_learning_curve(modelDT, metric, "XGB-default", x_train, y_train)

    modelDT = XGBClassifier(random_state=0, max_depth=4, max_leaf_nodes=10, ccp_alpha=.0025)

    create_learning_curve(modelDT, metric, "XGB-maxDepth=4 leaf_nodes=10", x_train, y_train)
    find_alph(modelDT)
    modelDT = XGBClassifier(random_state=0, max_depth=4, max_leaf_nodes=10, ccp_alpha=.0025)
    create_learning_curve(modelDT, metric, "XGB-tuned-postPruned", x_train, y_train)
...
def find_alph(clf):
    ...
    for ccp_alpha in ccp_alphas:
        clf = XGBClassifier(random_state=0, ccp_alpha=ccp_alpha)
...
def GrSearch():
    modelDT = XGBClassifier(random_state=0, ccp_alpha=.003)
